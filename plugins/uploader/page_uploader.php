...
{
	if($loguser['powerlevel'] > 2)
		$entries = Query("select {uploader}.*, {users}.name, {users}.displayname, {users}.powerlevel, {users}.sex, {users}.color, {users}.hascolor from {uploader} left join {users} on {uploader}.user = {users}.id where {uploader}.private = 1 order by user, ".$skey.$sdir);
	else
		$entries = Query("select {uploader}.*, {users}.name, {users}.displayname, {users}.powerlevel, {users}.sex, {users}.color, {users}.hascolor from {uploader} left join {users} on {uploader}.user = {users}.id where {uploader}.user = {0} and {uploader}.private = 1 order by ".$skey.$sdir, $loguserid);
...
$entries = Query("select 
						up.*, 
						u.name, u.displayname, u.powerlevel, u.sex, u.color, u.hascolor
					from {uploader} up
